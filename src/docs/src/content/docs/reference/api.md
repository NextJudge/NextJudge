...
| ------ | ---- | ----------- | ------- |
| POST | `/v1/basic_register` | none | Create account + JWT when `BASIC_REGISTRATION_ENABLED=true` |
...
| POST | `/v1/basic_request_password_reset` | none | Issue a one-time reset token; the trusted web bridge emails it without exposing it publicly |
