...
@pytest.fixture(scope="session")
def admin_token(host: str) -> str:
    return ensure_admin_token(host)

@pytest.fixture(scope="session", autouse=True)
def setup_tests(host: str, admin_token: str):
    globals_ = load_globals()
    admin_headers = {"Authorization": admin_token}
    test_users = [globals_["test_username"], globals_["test_username_2"]]
    for u in test_users:
    ...
    for p in safe_json_list(r):
        if p['title'] in (globals_["test_problem"], globals_["test_problem_2"]):
