...
import { UserSelector } from "@/app/platform/admin/contests/user-selector";
...
export function EnhancedContestCard({
	...
	const fetchParticipants = useCallback(async () => {
		...
		try {
			const eventParticipants = await apiGetEventParticipants(
				session.nextjudge_token,
				contest.id,
			);
	...
	return (
		<>
			<ContestCard
				...
				onAddParticipant={() => setShowParticipantDialog(true)}
							...
							{Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
								<PaginationItem key={page}>
									<Button
										...
										className={cn(currentPage === page && "bg-background")}
									>
										{page}
									</Button>
								</PaginationItem>
